...
export default function PoliticsPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      setIsLoading(true);
      const data = await getArticlesByCategory("Politics");
      if (isMounted) {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col font-sans selection:bg-[#E64A19] selection:text-white">
      ...
      {/* Topic Hero */}
      <section className="border-b border-[#2C2C2C] bg-gradient-to-b from-[#1E1410] to-[#0A0A0A] px-4 lg:px-8 py-8 lg:py-10">
        <div className="max-w-[1280px] mx-auto space-y-2">
          ...
          <p className="text-sm text-[#9E9E9E] max-w-2xl leading-relaxed">
            Real-time multi-angle news analysis covering elections, governance, international diplomacy, and institutional legislation with AI-estimated political framing.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 lg:px-8 py-3 border-b border-[#2C2C2C] bg-[#0E0E0E]">
        <div className="max-w-[1280px] mx-auto flex items-center justify-between">
          <span className="text-xs text-[#9E9E9E]">
            Showing <strong className="text-white">{articles.length}</strong> political perspectives
          </span>
      ...
      <main className="flex-1 max-w-[1280px] w-full mx-auto px-4 lg:px-8 py-8">
        {isLoading ? (
          <div className="bg-[#1E1E1E] rounded-xl border border-[#2C2C2C] p-12 text-center flex flex-col items-center justify-center space-y-3">
            ...
            <p className="text-xs font-mono text-[#9E9E9E]">Loading politics perspectives...</p>
          </div>
        ) : articles.length === 0 ? (
          <div className="bg-[#1E1E1E] rounded-xl border border-[#2C2C2C] p-12 text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-[#9E9E9E] max-w-sm mx-auto">
                No analyzed Politics articles in Supabase yet. Run the scraping and analysis pipeline to populate this category.
              </p>
            </div>
          </div>
        ) : (
