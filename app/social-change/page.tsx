...
export default function SocialChangePage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      setIsLoading(true);
      const data = await getArticlesByCategory("Social Change");
      if (isMounted) {
      ...
      <main className="flex-1 max-w-[1280px] w-full mx-auto px-4 lg:px-8 py-8">
        {isLoading ? (
          <div className="bg-[#1E1E1E] rounded-xl border border-[#2C2C2C] p-12 text-center flex flex-col items-center justify-center space-y-3">
            ...
            <p className="text-xs font-mono text-[#9E9E9E]">Loading social change perspectives...</p>
          </div>
        ) : articles.length === 0 ? (
          <div className="bg-[#1E1E1E] rounded-xl border border-[#2C2C2C] p-12 text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-[#9E9E9E] max-w-sm mx-auto">
                No analyzed Social Change articles in Supabase yet. Run the scraping and analysis pipeline to populate this category.
              </p>
            </div>
          </div>
        ) : (
